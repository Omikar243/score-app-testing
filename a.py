...
def main():
    ...
    feature_configs = {
        'Electricity': {'enable': st.sidebar.checkbox("Electricity", value=True), 
                       'range': st.sidebar.slider("Electricity Range (in kWh)", 0, 10000, (100, 5000))},
        'Water': {'enable': st.sidebar.checkbox("Water", value=True), 
                 'range': st.sidebar.slider("Water Range (in litres)", 0, 10000, (100, 3000))},
        'Gas': {'enable': st.sidebar.checkbox("Gas", value=False), 
               'range': st.sidebar.slider("Gas Range (in kgs)", 0, 1000, (10, 500))},
        'Waste': {'enable': st.sidebar.checkbox("Waste", value=False), 
                 'range': st.sidebar.slider("Waste Range (in kgs)", 0, 5000, (50, 1000))}
    }

    if st.sidebar.button("Generate Data"):
        data = generate_synthetic_data(num_records, feature_configs)
        scored_df=calculate_scores(data, feature_configs)
        ...
        try:
            ...
            if missing_features:
                ...
                if scored_user_df is not None:
                    user_dashboard = scored_user_df.sort_values(by='Score', ascending=False).reset_index(drop=True)
                    st.header("User Data Dashboard(Uploaded Data)")
                    st.dataframe(user_dashboard,height=400)
                    csv=user_dashboard.to_csv(index=False)
